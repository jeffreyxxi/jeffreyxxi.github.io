...
<li>
<p>call(),apply()和 bind() 函数内时， 参考 <RouteLink to="/lang/javascript.html#call-apply-bind">call(),apply(),bind()</RouteLink></p>
</li>
<li>
...
<p><strong><span style="color:#F56CC6">注意!!! 添加的属性的描述符的配置项的默认值如果不配置的话,可能会影响对象的使用,比如不可以写不可枚举等.</span></strong></p>
...
<p>冻结：不能添加新的属性, 不能删除已有属性,不能修改已有属性的可枚举型、可配置性、可写性和已有属性的值，并且不能修改原型。
被冻结对象的所有属性都不可能以任何方式被修改，包括值，访问器属性。但如果一个属性的值是个对象，则这个对象的属性是可以修改（除非这个对象也是一个被冻结的对象）。</p>
